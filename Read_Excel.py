...
try:
    ...
    # 2. تنظيف البيانات: حذف الصفوف الفارغة تماماً
    df = df.dropna(how='all')

    # 3. حذف الصفوف التي تحتوي على خانات فارغة في أعمدة أساسية (مثل العمود الأول والثاني)
    # هذا سيخفي الصفوف التي تحتوي على معادلات لكن بدون نتائج حقيقية
    df = df.dropna(subset=[df.columns[0], df.columns[1]])

    # 4. ترتيب البيانات حسب النقاط (بافتراض العمود الثاني هو النقاط)
